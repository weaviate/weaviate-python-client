...
class WeaviateClient:
    ...
    def connect(self) -> None:
        ...
        self._event_loop.run_until_complete(self._connection.connect, self.__skip_init_checks)
    ...
    def is_live(self) -> bool:
        try:
            response = self._event_loop.run_until_complete(
                self._connection.get, path="/.well-known/live"
            )
    ...
    def is_ready(self) -> bool:
        try:
            response = self._event_loop.run_until_complete(
                self._connection.get, path="/.well-known/ready"
            )
            if response.status_code == 200:
                return True
            return False
    ...
    def get_meta(self) -> dict:
        ...
        return self._event_loop.run_until_complete(self._connection.get_meta)
...
class WeaviateAsyncClient:
    ...
    async def connect(self) -> None:
        ...
        if self._connection.is_connected():
            return
        await self._connection.connect(self.__skip_init_checks)
