...
class _ConfigCollectionAsync:
    ...
    async def __get(self) -> Dict[str, Any]:
        response = await self.__connection.get(
    ...
    async def _get_reference_by_name(self, reference_name: str) -> Optional[_ReferenceProperty]:
        for ref in (await self.get()).references:
    ...
    async def update_shards(
        ...
        if shard_names is None:
            shards_config = await self.get_shards()
    ...
    async def add_property(self, prop: Property) -> None:
        ...
        if (await self._get_property_by_name(prop.name)) is not None:
    ...
    async def add_reference(
        ...
        if (await self._get_reference_by_name(ref.name)) is not None:
            raise WeaviateInvalidInputError(
