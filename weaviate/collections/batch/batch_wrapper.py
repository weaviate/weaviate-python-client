...
T = TypeVar("T", bound=Union[_BatchBase, _BatchBaseSync, _BatchBaseAsync])
...
class _ContextManagerSync:
    def __init__(self, current_batch: _BatchBaseSync):
        self.__current_batch = current_batch

    def __exit__(self, exc_type: Any, exc_val: Any, exc_tb: Any) -> None:
        self.__current_batch._shutdown()

    def __enter__(self) -> _BatchBaseSync:
        self.__bg_tasks = self.__current_batch._start()
        return self.__current_batch


class _ContextManagerAsync:
    ...
    async def __aenter__(self) -> _BatchBaseAsync:
