...
class _ConnectionBase:
    ...
    def _ping_grpc(self, colour: executor.Colour) -> Union[None, Awaitable[None]]:
        ...
        try:
            res = self._grpc_channel.unary_unary(
                ...
                request_serializer=health_weaviate_pb2.HealthCheckRequest.SerializeToString,
                response_deserializer=health_weaviate_pb2.HealthCheckResponse.FromString,
            )(health_weaviate_pb2.HealthCheckRequest(), timeout=self.timeout_config.init)
            if colour == "async":
                ...
                async def execute():
                    ...
                    try:
                        return self.__handle_ping_response(
                            cast(health_weaviate_pb2.HealthCheckResponse, await res)
                        )
            ...
            self.__handle_ping_response(cast(health_weaviate_pb2.HealthCheckResponse, res))
            return None
    ...
    def __handle_ping_response(self, res: health_weaviate_pb2.HealthCheckResponse) -> None:
        if res.status != health_weaviate_pb2.HealthCheckResponse.SERVING:
