...
def test_filters_text(
    client: weaviate.CollectionClient, weaviate_filter: _FilterValue, results: List[int]
):
...
def test_length_filter(client: weaviate.CollectionClient):
    client.collection.delete("TestFilterNested")
    collection = client.collection.create(
        name="TestFilterNested",
        vectorizer_config=ConfigFactory.Vectorizer.none(),
...
def test_ref_filters_multi_target(client: weaviate.CollectionClient):
...
def test_delete_many_and(client: weaviate.CollectionClient):
...
def test_delete_many_or(client: weaviate.CollectionClient):
...
def test_delete_many_return(client: weaviate.CollectionClient):
