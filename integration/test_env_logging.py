...
def mock_client(monkeypatch):
    ...
    async def mock_create_collection(*args, **kwargs):
        ...
        async def mock_insert(data):
            ...
            return await client._connection.post(
                path="/v1/objects",
                weaviate_object=object_data
            )
    ...
    client.collections.delete = AsyncMock(side_effect=lambda name: 
        client._connection.delete(path=f"/v1/schema/{name}"))
...
@pytest.mark.asyncio
async def test_debug_logging_enabled(mock_client, log_capture: LogCaptureHandler, monkeypatch) -> None:
...
@pytest.mark.asyncio
async def test_debug_logging_async(mock_client, log_capture: LogCaptureHandler, monkeypatch) -> None:
    ...
    assert any("Response: status_code=" in log for log in log_capture.logs), "No HTTP response logs found"
    ...
    try:
        ...
        assert any("gRPC Response" in log for log in log_capture.logs), "No gRPC response logs found"
        assert any("Method: Search" in log for log in log_capture.logs), "gRPC method name not logged"
        assert any("TestAsyncGrpcLogging" in log for log in log_capture.logs), "Collection name not found in gRPC logs"
...
async def test_concurrent_logging(
    ...
    try:
        ...
        for name in collection_names:
            ...
            assert_log_contains(f'POST http://localhost:8080/v1/v1/schema', f"Schema creation request not logged for {name}")
            
            # Verify data insertion logs
            assert_log_contains(f'test_{name}', f"Data insertion not logged for {name}")
            assert_log_contains(f'POST http://localhost:8080/v1/v1/objects', f"Object creation request not logged for {name}")
