...
def test_low_timeout():
    ...
    num_objects = 5000  # cannot be increased too high, because weaviate can't return that many results
    uuids = []
    for i in range(num_objects):
        uuids.append(uuid.uuid4())
        client.batch.add_data_object({"stringProp": f"object-{i}", "intProp": i}, "ClassA", uuid=uuids[-1])
    client.batch.flush()
    time.sleep(0.1)
    result = client.query \
        .aggregate("ClassA") \
    ...
    for i in range(num_objects):
        client.batch.add_data_object({"stringProp": f"object-{i*2}", "intProp": i*2}, "ClassA", uuid=uuids[i])
    client.batch.flush()
    ...
    result = client.query \
        .get("ClassA", ["intProp"]).with_additional("id").with_limit(num_objects+10).do()
