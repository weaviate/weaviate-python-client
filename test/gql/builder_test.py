...
class TestGraphQLBuilder(unittest.TestCase):
    ...
    def test_build_input(self):
        try:
            Builder(1, ["a"], None, "Things").build()
            self.fail("expected error")
        except TypeError:
            pass
        try:
            Builder("A", 2, None, "Things").build()
            self.fail("expected error")
        except TypeError:
            pass

    def test_build_simple_query(self):
        query = Builder("Group", "name", None, "Things").build()
        self.assertEqual("{Get{Things{Group{name}}}}", query)

        query = Builder("Group", ["name", "uuid"], None, "Things").build()
        self.assertEqual("{Get{Things{Group{name uuid}}}}", query)

        query = Builder("Group", ["name", "uuid"], None, "Actions").build()
        self.assertEqual("{Get{Actions{Group{name uuid}}}}", query)

    def test_build_limited_query(self):
        query = Builder("Person", "name", None, "Things").with_limit(20).build()
    ...
    def test_build_where_limited_query(self):
        ...
        query = Builder("Person", "name", None, "Things").with_limit(1).with_where(filter).build()
    ...
    def test_build_explore_query(self):
        ...
        query = Builder("Person", "name", None, "Things").with_explore(explore).build()
    ...
    def test_build_full_query(self):
        ...
        query = Builder("Person", ["name", "uuid"], None, "Things").with_explore(explore).with_where(filter).with_limit(2).build()
