...
class TestWCS(unittest.TestCase):
    ...
    def test_create(self, mock_get_cluster_config, mock_post):
        ...
        config = {
            ...
            "configuration": {"tier": "sandbox", "requiresAuthentication": False},
        }
        ...
        check_error_message(
            self, error, f'"{key_error_message(modules)}"'
        )  # KeyError adds extra quotes
        ...
        modules = [
            "test",
            {"name": "test2", "repo": "test_repo", "tag": "TAG", "inferenceUrl": "URL"},
        ]
        ...
        mock_get_cluster_config.side_effect = (
            lambda x: progress("weaviate", 100)
            if mock_get_cluster_config.call_count == 2
            else progress("weaviate")
        )
