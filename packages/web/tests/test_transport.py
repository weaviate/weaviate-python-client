...
async def test_unary_success_round_trip():
    ...
    result = await mc(b"request-bytes", metadata=[("authorization", "Bearer k")], timeout=5)
...
async def test_secure_channel_uses_https():
...
async def test_health_call_without_metadata():
    ...
    assert await mc(b"ping", timeout=2) == b"pong"


async def test_error_trailer_raises_aiorpcerror():
...
async def test_percent_encoded_grpc_message_decoded():
...
async def test_trailers_only_status_in_http_headers():
    channel = _channel(
...
async def _details_of(status, body, headers=None, path="/grpc.health.v1.Health/Check"):
...
async def test_nginx_404_html_is_reported_as_an_http_404():
    err = await _details_of(404, NGINX_404_HTML)
    assert err.code() is StatusCode.UNIMPLEMENTED
...
async def test_spa_html_body_is_not_reported_as_truncated():
...
async def test_message_frame_after_trailer_is_internal():
...
async def test_error_status_wins_over_multiple_message_frames():
...
async def test_spa_fallback_html_200_is_distinguishable_from_a_404():
    ...
    assert details != (await _details_of(404, NGINX_404_HTML)).details()


async def test_401_json_body_maps_to_unauthenticated():
    err = await _details_of(401, b'{"error":[{"message":"anonymous access not enabled"}]}')
...
async def test_error_body_excerpt_is_capped():
    err = await _details_of(500, b"E" * 5000)
    details = err.details()
...
async def test_invalid_utf8_grpc_message_preserves_the_status():
...
async def test_binary_metadata_base64_encoded():
    sender = FakeSender(body=_ok_response(b"x"))
    channel = _channel(sender)
    mc = channel.unary_unary("/svc/M", lambda x: x, lambda b: b)
    await mc(b"q", metadata=[("trace-bin", b"\x00\x01\x02")])
...
async def test_empty_ok_response_hints_at_cors_expose_headers():
...
async def test_message_frame_with_grpc_status_header_still_succeeds():
    ...
    mc = channel.unary_unary("/svc/M", lambda x: x, lambda b: b)
    assert await mc(b"q") == b"reply-bytes"
...
async def test_malformed_frame_maps_to_internal():
...
async def test_malformed_grpc_status_maps_to_internal():
...
async def test_grpc_timeout_header_rounds_up():
    sender = FakeSender(body=_ok_response(b"x"))
    channel = _channel(sender)
    mc = channel.unary_unary("/svc/M", lambda x: x, lambda b: b)
...
async def test_infinite_timeout_sends_no_deadline():
    ...
    channel = _channel(sender)
    mc = channel.unary_unary("/svc/M", lambda x: x, lambda b: b)
    assert await mc(b"q", timeout=float("inf")) == b"x"
...
async def test_huge_timeout_uses_minutes_then_no_deadline():
    ...
    channel = _channel(sender)
    mc = channel.unary_unary("/svc/M", lambda x: x, lambda b: b)
    await mc(b"q", timeout=1e8)
    await mc(b"q", timeout=1e9)
    await mc(b"q", timeout=1e10)
...
async def test_close_is_awaitable_noop():
    channel = _channel(FakeSender())
    assert await channel.close() is None


async def test_path_prefix_prepended_to_url():
